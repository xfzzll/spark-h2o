...
import org.apache.spark.ml.Tabulizer.table
import org.apache.spark.h2o._


object Example extends App {
    val config = new SparkConf().setMaster("spark://master:7077").setAppName("ctrust-spark")
    val sc = new SparkContext(config)
    val spark:SparkSession = SparkSession.builder().config(config).getOrCreate()
    val rawData = sc.textFile("/opt/spark/examples/first/higgs100k.csv")
    println(s"Number of rows: ${rawData.count}")
    println("Rows" + "\n" + rawData.take(2).mkString("\n"))

    val data = rawData.map(line => line.split(',').map(_.toDouble))

    val response: org.apache.spark.rdd.RDD[Int] = data.map(row => row(0).toInt)

    val features: org.apache.spark.rdd.RDD[Vector] = data.map(line => Vectors.dense(line.slice(1, line.size)))
    ...
    def computeMetrics(model: Predictor, data: org.apache.spark.rdd.RDD[LabeledPoint]): BinaryClassificationMetrics = {
    ...
    // @Snippet
    def computeError(model: Predictor, data: org.apache.spark.rdd.RDD[LabeledPoint]): Double = {
   ...
   """.stripMargin('~'))
    ...
    // @Snippet
    import water.support.ModelMetricsSupport.{modelMetrics => execute }
    import _root_.hex.{ModelMetricsBinomial => Model}
    val dlMetrics = execute[Model](dlModel, testHF)
